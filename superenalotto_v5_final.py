# SUPERNALOTTO ENGINE V5.1 FINAL
# Discovery 2016-2021 -> Bonferroni -> optional Confirmation 2022-2026
# -> candidate tickets. No claim of predictive power.
...
def parse_month(y,m):
    url=f'{BASE_URL}/{y}/{MONTHS[m]}'; print(f'WEB {y}-{m:02d}',flush=True)
    try:
        r=S.get(url,timeout=15)
        if r.status_code!=200: return []
        lines=[x.strip() for x in BeautifulSoup(r.text,'html.parser').get_text('\n',strip=True).splitlines() if x.strip()]
    except Exception as e:
        print(' HTTP:',e,flush=True); return []
...
def archive():
    if os.path.exists(OUTPUT):
        ...
        if not df.empty and all(c in df for c in req):
            print(f'LOCAL CSV: {len(df)} draws',flush=True); return df
    rows=[]; failed=[]; now=pd.Timestamp.now()
    for y in range(FIRST_YEAR,LAST_YEAR+1):
        maxm=now.month if y==now.year else 12
        for m in range(1,maxm+1):
            r=parse_month(y,m); rows+=r
            if not r: failed.append(f'{y}-{m:02d}')
            time.sleep(.1)
    if not rows: raise RuntimeError('Nessun dato scaricato.')
    df=pd.DataFrame(rows).sort_values(['concorso','data'])
    # conflicting duplicate contest numbers are fatal
    for c,g in df.groupby('concorso'):
        if len(g[['data','n1','n2','n3','n4','n5','n6']].drop_duplicates())>1:
            raise RuntimeError(f'Concorso duplicato con dati discordanti: {c}')
    return df.drop_duplicates('concorso').sort_values(['data','concorso']).reset_index(drop=True)

def validate(df):
    req=['concorso','data','n1','n2','n3','n4','n5','n6']
    if any(c not in df for c in req): raise ValueError('Colonne mancanti')
    df=df.copy(); df['data']=pd.to_datetime(df['data'],errors='coerce')
    if df.data.isna().any(): raise ValueError('Date non valide')
    bad=[]
    for i,r in df.iterrows():
        ns=[int(r[f'n{k}']) for k in range(1,7)]
    ...
    pd.DataFrame([{'check':'rows_final','value':len(df)},{'check':'invalid_removed','value':len(bad)},{'check':'first_date','value':str(df.data.min().date())},{'check':'last_date','value':str(df.data.max().date())}]).to_csv(QUALITY,index=False)
    df.to_csv(OUTPUT,index=False); print(f'ARCHIVE OK: {len(df)} draws | {df.data.min().date()} -> {df.data.max().date()}',flush=True); return df

class Engine:
    def __init__(self,df):
        df=df.sort_values(['data','concorso']).reset_index(drop=True); self.df=df
        self.draws=df[[f'n{i}' for i in range(1,7)]].to_numpy(np.int16); self.dates=df.data.to_numpy(); self.contests=df.concorso.to_numpy()
    @staticmethod
    def state(hist):
    ...
    def oos(self,st,start,end,seed):
        ix=np.where((self.dates>=pd.Timestamp(start))&(self.dates<=pd.Timestamp(end)))[0]; first=ix[0]
        if first<MIN_HISTORY: raise ValueError('Storico insufficiente')
        r=np.random.default_rng(seed); f,d=self.state(self.draws[:first]); total=0; rows=[]
        for i in ix:
    ...
    def null(self,st,start,end,n,seed):
        ix=np.where((self.dates>=pd.Timestamp(start))&(self.dates<=pd.Timestamp(end)))[0]; first=ix[0]; hist=self.draws[:first]; L=len(ix); master=np.random.default_rng(seed); out=np.empty(n,np.int32); nums=np.arange(1,91); t0=time.time()
        for s in range(n):
            r=np.random.default_rng(int(master.integers(0,2**63-1))); f,d=self.state(hist); total=0
            for _ in range(L):
                ticket=self.pick(st,f,d,r); actual=np.sort(r.choice(nums,6,False)); total+=len(set(ticket)&set(actual)); self.update(actual,f,d)
            out[s]=total
    ...
    def phase(self,name,start,end,strats,n,seed):
        ...
        for sid,st in enumerate(strats):
            ss=seed+sid*100000; obs,L,rows=self.oos(st,start,end,ss); logs+=rows
            nul=self.null(st,start,end,n,ss+999999); p=(np.sum(nul>=obs)+1)/(n+1); passed=p<alpha
    ...
    def run(self):
        disc,logs1=self.phase('DISCOVERY OOS',DISCOVERY_START,DISCOVERY_END,STRATEGIES,N_NULL,SEED); disc.to_csv(DISC,index=False)
        passed=disc.loc[disc.passed,'strategy'].tolist(); logs=logs1
        if passed:
            conf,logs2=self.phase('CONFIRMATION OOS',CONFIRMATION_START,CONFIRMATION_END,passed,N_NULL,SEED+500000); logs+=logs2; pd.concat([disc,conf],ignore_index=True).to_csv(FULL,index=False)
            confirmed=conf.loc[conf.passed,'strategy'].tolist(); st=confirmed[0] if confirmed else passed[0]
            print('ESITO:', 'SEGNALE STATISTICO CONFERMATO - DA APPROFONDIRE' if confirmed else 'SEGNALE NON REPLICATO - NESSUNA EVIDENZA ROBUSTA',flush=True)
        else:
            pd.DataFrame(disc).to_csv(FULL,index=False); st='MIXED'; print('ESITO: H0 NON VIENE RIGETTATA - NESSUNA EVIDENZA STATISTICA OLTRE IL CASO',flush=True)
        pd.DataFrame(logs).to_csv(OOS,index=False); t=self.tickets(st,N_TICKETS,SEED+900000); t.to_csv(TICKETS,index=False)
        print('\nSESTINE CANDIDATE (non garantite):',flush=True)
        print('\n'.join(f"{int(r['rank']):02d}) {r['ticket']}" for _,r in t.iterrows()),flush=True)
        print('\nFILE:',OUTPUT,QUALITY,DISC,FULL,OOS,TICKETS,flush=True)

if __name__=='__main__':
    try: Engine(validate(archive())).run()
    except Exception as e: print(f'ERRORE PIPELINE: {e}',flush=True); raise
